...
def main():
    ...
    for dt in dates:
        ...
        if current_theme is not None:
            raw_cycle_r = proxy.loc[dt, current_theme]
        ...
        ret = (
            anchor_weight * anchor_r
            + rf_weight * RISK_FREE_DAILY
            + cycle_weight * cycle_r
        )
        ...
        if not np.isfinite(ret):
            raise ValueError(
                ...
                f"cycle_weight={cycle_weight}, "
                f"anchor_weight={anchor_weight}, "
                f"rf_weight={rf_weight}, "
                f"anchor_r={anchor_r}, "
                f"cycle_r={cycle_r}"
            )
    ...
    summary = pd.DataFrame([{
        "strategy": "BASE_SEQUENCE_STEP_UP_TRAILING_EXIT_CPPI_CEILING",
        ...
        "num_sequence_confirms": int((trades_df["action"] == "SEQUENCE_CONFIRMED_OPEN_STEP_RISK").sum()) if not trades_df.empty else 0,
    ...
    print("\n===== BASE + SEQUENCE + STEP-UP + CPPI CEILING SUMMARY =====")
