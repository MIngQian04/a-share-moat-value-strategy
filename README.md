...
图表在“今日、5日、1个月、6个月、1年和累计”视图中，只要该周期有至少两个共同记录日，就会同时绘制策略净值和沪深300基准线。非累计周期会从各自周期的第一天重新归一化，避免把不同观察区间混在一起；共同记录日不足时只显示策略线，不会用缺失数据补齐大盘。
...
累计视图还会把组合起始日的沪深300（`000300.SH`）原始收盘价归一化为单位 `1.0000`，与策略单位净值并列比较，并显示相对大盘的累计超额收益。该基准不含指数分红；若同一记录日的数据缺失，网站会显示基准不完整，不会用组合收益补齐。
...
- 含分红的真实前瞻净值曲线；
- 从组合起始日单位1开始的策略 vs 沪深300累计对比曲线和超额收益；
- 每位访问者浏览器本地保存的个人起始日；
- 每只持仓的动态护城河档案和待复核事件；
- 首次访问使用说明、调仓原因弹窗和中英文界面；
- 当日生效仓位与明日 T+1 目标的左右滑动对照；
- 每个可用周期的策略与沪深300走势对照，以及累计相对大盘超额收益；
- 模型信号与个人真实成交分离的成交记录区；
- 中文/英文切换与移动端适配。
